 ...
 *
 * ValueAnimator - 值控制器，继承自 Animator（在 xml 定义的各种值控制器均需要放在 res/animator 目录下）
 *     ofInt(int... values) - 创建 ValueAnimator 用于控制指定的 int 类型的值（内置 TypeEvaluator）
 *     ofFloat(float... values) - 创建 ValueAnimator 用于控制指定的 float 类型的值（内置 TypeEvaluator）
 *     ofArgb(int... values) - 创建 ValueAnimator 用于控制指定的 color 类型的值（内置 TypeEvaluator）
 *     ofObject(TypeEvaluator evaluator, Object... values) - 创建 ValueAnimator 用于控制指定的 object 类型的值（需要自定义 object 的 TypeEvaluator）
...
public class AnimationDemo5 extends AppCompatActivity {
    ...
    private void sample1() {
        ...
        // 下面这个是控制 float 类型的动画的，其他的还有 ofInt(), ofArgb(), ofObject()
    ...
    private void sample2() {
        ...
        // 创建 ValueAnimator 对象，指定 TypeEvaluator（参见 ui/AnimationDemo5CustomTypeEvaluator.java）和动画的起始点
