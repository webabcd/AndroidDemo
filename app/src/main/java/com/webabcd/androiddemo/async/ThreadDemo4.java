 ...
 * 3、调用了线程的 interrupt() 后，分两种情况，一个是会触发 InterruptedException 异常，一个是会改变 interrupted 状态，参见下面的 sample1 和 sample2 示例
...
public class ThreadDemo4 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 注：此时线程的 interrupted 状态仍然为 false
        sample1();
    ...
    private void sample1() {
        Thread thread1 = new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    try {
                        // sleep 过程中被 interrupt() 则触发 InterruptedException 异常，线程的 interrupted 状态仍然为 false
     ...
     * thread2 isInterrupted:true
     * thread2 isInterrupted:true
     * thread2 interrupted: true //  interrupt() 之后，i第一次 Thread.interrupted() 的值为 true，与此同时会将 interrupted 置为 false
